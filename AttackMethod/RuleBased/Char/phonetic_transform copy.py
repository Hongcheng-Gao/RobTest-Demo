...
import torch
import os
# phonetic_dict_path = os.path.join(os.path.dirname(__file__), 'phonetic_dict_0.1.pt')
...
class PhoneticTransform(RuleTransform):
    def __init__(self, degree, aug_num=1,distance_type="char",ori_sent=""):
        ...
        self.anthro.load('AttackMethod/RuleBased/Char/anthro/ANTHRO_Data_V1.0')
        # print('loaded.')
        # self.candidate_words_dict = torch.load(phonetic_dict_path) if os.path.exists(phonetic_dict_path) else {}
    ...
    def phonetic_transform(self, word, case_sensitive=True):
        ...
        # start = time.time()
        distance = math.ceil(len(word)*self.degree) if self.dis_type=='char' else 1
        name = word+'-'+str(distance)
    ...
    def transform_target(self, sent,sent_idx, case_sensitive=True):
        ...
        self.trans = self.phonetic_transform(word)
        substitute = self.trans(word, case_sensitive=case_sensitive)
        if substitute == None:
            return None
